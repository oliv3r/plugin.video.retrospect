...
MOCK_PROFILE_ID = "AAAAAAAA-BBBB-CCCC-DDDD-222222222222"
...
MOCK_TOKEN_RESPONSE_WITH_REFRESH: Dict[str, Any] = {
    "id_token": MOCK_ID_TOKEN,
    "access_token": MOCK_ACCESS_TOKEN,
...
MOCK_PROFILE_TOKEN_RESPONSE: Dict[str, Any] = {
    "id_token": MOCK_ID_TOKEN,
...
MOCK_DEVICE_AUTH_RESPONSE: Dict[str, Any] = {
...
MOCK_USERINFO_RESPONSE: Dict[str, Any] = {
    "sub": MOCK_USER_ID,
    "email": MOCK_EMAIL,
    "name": "Test User",
    "email_verified": True
...
MOCK_EMPTY_SESSION_RESPONSE: Dict[str, Any] = {"sessions": []}

MOCK_SESSION_RESPONSE: Dict[str, Any] = {"sessions": [{
...
MOCK_DEVICE_PENDING_RESPONSE: Dict[str, Any] = {"error": "authorization_pending"}
...
class NLZietMockDispatcher:
    """Routes UriHandler.open() calls to canned responses for authentication tests."""
    ...
    def dispatch(self, uri: str, uri_handler_instance: Any, **kwargs: Any) -> str:
        ...
        if "id.nlziet.nl" in host:
            if (path == "/connect/deviceauthorization" and
                data):
            ...
            if ("/api/session/revoke/" in uri and
                method == "DELETE"):
            ...
            if path == "/connect/endsession":
                uri_handler_instance.status = UriStatus(code=302, url=uri, error=False, reason="Found")
                return ""
    ...
    def _handle_authorize(self, uri: str, uri_handler_instance: Any, parsed: Any) -> str:
        ...
        if prompt == "none":
            ...
            uri_handler_instance.status = UriStatus(
                code=200, url=callback_url, error=False, reason="OK")
            return ""
        ...
        login_url = f"https://id.nlziet.nl/account/login?ReturnUrl={quote(return_url)}"
        uri_handler_instance.status = UriStatus(code=200, url=login_url, error=False, reason="OK")
        return login_html


    def _handle_login_post(self, uri: str, uri_handler_instance: Any, params: Any) -> str:
        ...
        return_url_params = parse_qs(
            return_url.split("?", 1)[-1] if "?" in return_url else return_url)
    ...
    def _handle_token(self, uri: str, uri_handler_instance: Any, data: Any) -> str:
    ...
    def _handle_device_authorization(self, uri: str, uri_handler_instance: Any, data: Any) -> str:
        uri_handler_instance.status = UriStatus(code=200, url=uri, error=False, reason="OK")
        self.device_flow_authorized = False
        self.device_poll_count = 0
    ...
    def _handle_device_page_get(self, uri: str, uri_handler_instance: Any) -> str:
        uri_handler_instance.status = UriStatus(code=200, url=uri, error=False, reason="OK")
        return MOCK_DEVICE_PAGE_HTML


    def _handle_device_page_post(self, uri: str, uri_handler_instance: Any, params: Any) -> str:
        if isinstance(params, str):
            form_data = parse_qs(params)
        ...
        uri_handler_instance.status = UriStatus(
            code=200, url=MOCK_DEVICE_CONFIRMED_URL, error=False, reason="OK")
        return MOCK_DEVICE_CONFIRMED_HTML


    def _handle_session_list(self, uri: str, uri_handler_instance: Any) -> str:
        uri_handler_instance.status = UriStatus(code=200, url=uri, error=False, reason="OK")

        if (self._device_removed or
            not self._device_created):
            return json.dumps(MOCK_EMPTY_SESSION_RESPONSE)

        # Use the mock token iat (1700000000) as the device createdAt so that
        # _find_device_session_key's drift check passes when comparing against
        # the id_token iat claim.
        mock_ts = datetime.datetime.fromtimestamp(1700000000, tz=datetime.timezone.utc)
        ts_str = mock_ts.strftime("%Y-%m-%dT%H:%M:%S.000+00:00")
        session = dict(MOCK_SESSION_RESPONSE["sessions"][0])
        session["name"] = self._device_name
        session["createdAt"] = ts_str
        session["updatedAt"] = ts_str
        return json.dumps({"sessions": [session]})


    def _handle_session_revoke(self, uri: str, uri_handler_instance: Any) -> str:

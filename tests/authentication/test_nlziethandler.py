...
def submit_device_code(handler: NLZIETOAuth2Handler, user_code: str, device_name: str) -> bool:
    ...
    try:
        ...
        if 'account/login' in uri_handler.status.url.lower() or 'ReturnUrl' in html:
...
class TestNLZIETAuthLive(unittest.TestCase):
    ...
    def test_07_log_off_cleanup(self):
    ...
    def test_08_device_flow_initiation(self):
    ...
    def test_09_device_flow_authentication(self):
    ...
    def _run_device_flow_authentication(self):
        ...
        Logger.info("Step 5: Activating device session...")
        sessions = device_handler.list_devices(access_token=device_handler.get_valid_token())
        self.assertIsNotNone(sessions, "Failed to activate device session via /api/session")
        Logger.info("Device session activated")
    ...
    def _get_mijn_nlziet_token(self):
        """Get access token for triple-web OAuth client via silent auth.
        ...
        try:
            ...
            auth_params = {
                ...
                'redirect_uri': 'https://app.nlziet.nl/callback-silent.html',
                'response_type': 'code',
                'scope': 'openid api',
                'state': state,
            ...
            Logger.warning(f"Failed to get triple-web token: {e}")
            return None

    @unittest.skipIf(os.getenv('TEST_NLZIET_DEVICE_FLOW', 'AUTO') not in ('AUTO', 'MANUAL'),
                     "Set TEST_NLZIET_DEVICE_FLOW=AUTO or MANUAL to enable device flow test")
    def test_10_device_flow_refresh_token(self):
    ...
    def _run_device_flow_refresh_token(self):
        ...
        if not refresh_token:
            ...
            device_handler.list_devices(access_token=device_handler.get_valid_token())
    ...
    def test_11_appconfig_status(self):
...
class TestNLZIETAuthMocked(TestNLZIETAuthLive):
    ...
    def test_get_valid_token_refreshes_when_expired(self):
        """get_valid_token() proactively refreshes the token when it is near expiry."""
        from tests.authentication.nlziethandler_mocks import MOCK_ACCESS_TOKEN
        self.handler._access_token = MOCK_ACCESS_TOKEN
        self.handler._expires_at = 0  # expired — get_valid_token must refresh
        ...
        self.assertNotEqual(refresh_called, [], "_do_token_refresh MUST be called when token is expired")
    ...
    def test_properties_correctness(self):
        ...
        web_handler = NLZIETOAuth2Handler(use_device_flow=False)
        self.assertEqual(web_handler.client_id_val, "triple-web",
                         "Web flow must use triple-web client ID")

    def test_log_on_with_device_flow_wrapper(self):
        """Test the high-level log_on_with_device_flow wrapper logic."""
        device_handler = NLZIETOAuth2Handler(use_device_flow=True)
        ...
        mock_flow_data = {
            ...
            "verification_uri_complete": "https://example.com/verify?code=123-456",
            "expires_in": 300,
            "interval": 5
        }

        original_start = device_handler.start_device_flow
        original_poll = device_handler.poll_device_flow

        try:
            device_handler.start_device_flow = lambda name, client_id=None: mock_flow_data

            def mock_poll(code, interval, expires, max_attempts=0):
                device_handler._access_token = "mock_access_token"
                device_handler._expires_at = int(time.time()) + 3600
                return True

            device_handler.poll_device_flow = mock_poll

            callback_called = {}

            def mock_callback(code, uri, uri_complete):
                callback_called['code'] = code
                callback_called['uri'] = uri

            result = device_handler.log_on_with_device_flow("Test Device", display_callback=mock_callback)

            self.assertTrue(result.logged_on, "Should be logged on after successful flow")
            self.assertEqual(callback_called['code'], "123-456")
            self.assertEqual(callback_called['uri'], "https://example.com/verify")

        finally:
            device_handler.start_device_flow = original_start
            device_handler.poll_device_flow = original_poll


    @unittest.mock.patch('resources.lib.urihandler.UriHandler.open')
    def test_request_token_generic(self, mock_open):
        """request_token performs a generic token request with custom headers."""
        mock_open.return_value = '{"access_token": "generic_token", "expires_in": 3600}'

        data = {"grant_type": "custom_grant", "client_id": "client"}
        headers = {"Custom-Header": "value"}

        self.handler.request_token(data, headers=headers)

        # Verify call args
        args, kwargs = mock_open.call_args
        self.assertEqual(kwargs['data'], data)
        self.assertEqual(kwargs['additional_headers'], headers)

        # Verify token stored
        self.assertEqual(self.handler.get_valid_token(), "generic_token")

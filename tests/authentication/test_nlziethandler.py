...
from resources.lib.authentication.nlziethandler import (
    API_ID_AUTHORIZE, DEVICE_FLOW_USER_AGENT, NLZIETHandler,
    WEB_CLIENT_ID)
...
class TestNLZIETAuthLive(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.handler = NLZIETHandler(use_device_flow=False)
    ...
    def test_02_preflight_invalid_password(self) -> None:
        ...
        result = self.handler.log_on(self.username, MOCK_INVALID_PASSWORD)  # type: ignore[arg-type]
    ...
    def test_03_log_on_headless_success(self) -> None:
        """Main Integration: Full headless flow against NLZIET."""

        Logger.info(f"NLZIET: Testing live log_on for {self.username}")

        result = self.handler.log_on(self.username, self.password)  # type: ignore[arg-type]
    ...
    def test_06_token_refresh(self) -> None:
        ...
        expiry_key = f"{self.handler._prefix}access_token_expires_at"
    ...
    def test_07_log_off_cleanup(self) -> None:
        ...
        success = self.handler.log_off(self.username)  # type: ignore[arg-type]
    ...
    def test_08_device_flow_initiation(self) -> None:
        ...
        device_handler = NLZIETHandler(use_device_flow=True)

        # Generate unique test device name
        device_name = generate_test_device_name()

        result = device_handler.start_nlziet_device_flow(device_name=device_name)

        self.assertIsNotNone(result, "Device flow should return a result")
        self.assertIn("device_code", result)  # type: ignore[arg-type]
        self.assertIn("user_code", result)  # type: ignore[arg-type]
        self.assertIn("verification_uri", result)  # type: ignore[arg-type]
        self.assertEqual(result["verification_uri"], "https://nlziet.nl/koppel")  # type: ignore[index]
        self.assertIn("expires_in", result)  # type: ignore[arg-type]
        self.assertIn("interval", result)  # type: ignore[arg-type]

        Logger.info("=" * 60)
        Logger.info(f"Device flow initiated successfully!")
        Logger.info(f"Device name: {device_name}")
        Logger.info(f"User code: {result['user_code']}")  # type: ignore[index]
        Logger.info(f"Verification URL: {result['verification_uri']}")  # type: ignore[index]
    ...
    def _run_device_flow_authentication(self) -> None:
        ...
        device_info = device_handler.start_nlziet_device_flow(device_name=device_name)
        ...
        try:
            ...
            if 'account/login' in UriHandler.instance().status.url.lower():
                Logger.debug("No active session - logging in...")
                login_result = self.handler.log_on(self.username or "", self.password or "")
        ...
        end_time = time.time() + device_info.get('expires_in', 900)
        success = False
        while time.time() < end_time:
            poll_result = device_handler.poll_device_authorization(device_code)
        ...
        if device_flow_mode == 'AUTO':
            ...
            for key in ['access_token', 'refresh_token', 'id_token', 'access_token_expires_at']:
                AddonSettings.set_setting(f"{device_handler._prefix}{key}", "", store=LOCAL)
    ...
    def _run_device_flow_refresh_token(self) -> None:
        device_handler = NLZIETHandler(use_device_flow=True)
        refresh_token = AddonSettings.get_setting(f"{device_handler._prefix}refresh_token", store=LOCAL)
        ...
        if not refresh_token:
            ...
            if not self.handler.get_authentication_token():
                result = self.handler.log_on(self.username or "", self.password or "")
            ...
            success = False
            while time.time() < end_time:
                poll_result = device_handler.poll_device_authorization(device_info['device_code'])
                if poll_result == "success":
                    success = True
                    break
                if poll_result == "error":
            ...
            refresh_token = AddonSettings.get_setting(
                f"{device_handler._prefix}refresh_token", store=LOCAL)

        Logger.info("Testing device flow refresh token grant...")

        expiry_key = f"{device_handler._prefix}access_token_expires_at"
    ...
    def test_11_device_flow_log_off_removes_device(self) -> None:
        """Test that log_off deregisters the device from the account.
    ...
    def _run_device_flow_log_off(self) -> None:
        ...
        self.assertIsNotNone(devices_before, "Failed to list devices before log_off")
        ...
        Logger.info("Step 3: Calling log_off...")
        log_off_result = device_handler.log_off(self.username)  # type: ignore[arg-type]
        self.assertTrue(log_off_result, "log_off returned False")
        Logger.info("log_off returned True")

        Logger.info("Step 4: Verifying device was removed from account...")
        verifier = NLZIETHandler(use_device_flow=True)
        ...
        self.assertFalse(device_still_exists,
                         f"Device '{device_name}' still present after log_off")
        Logger.info(f"Verified device '{device_name}' was removed by log_off")
...
class TestNLZIETAuthMocked(TestNLZIETAuthLive):
    ...
    def test_refresh_no_tokens_raises_value_error(self) -> None:
        ...
        AddonSettings.set_setting(f"{self.handler._prefix}refresh_token", "", store=LOCAL)
        AddonSettings.set_setting(f"{self.handler._prefix}id_token", "", store=LOCAL)
    ...
    def test_instance_vars_loaded_from_settings_at_init(self) -> None:
        ...
        AddonSettings.set_setting(f"{self.handler._prefix}access_token", MOCK_ACCESS_TOKEN, store=LOCAL)
        AddonSettings.set_setting(
            f"{self.handler._prefix}refresh_token", MOCK_REFRESH_TOKEN, store=LOCAL)

        fresh = NLZIETHandler(use_device_flow=False)
    ...
    def test_id_token_loaded_for_device_flow_at_init(self) -> None:
        ...
        device_handler = NLZIETHandler(use_device_flow=True)
        AddonSettings.set_setting(f"{device_handler._prefix}id_token", MOCK_ID_TOKEN, store=LOCAL)

        fresh = NLZIETHandler(use_device_flow=True)
    ...
    def test_properties_correctness(self) -> None:
        ...
        web_handler = NLZIETHandler(use_device_flow=False)
    ...
    def test_deregister_device_clears_stored_key_on_success(self) -> None:
        """deregister_device() clears the stored key after a successful removal."""
        ...
        with unittest.mock.patch.object(device_handler, "_get_account_access_token", return_value="mock_account_token"):
            with unittest.mock.patch.object(device_handler, "_remove_device", return_value=True) as mock_remove:
                result = device_handler.deregister_device()
    ...
    def test_deregister_device_keeps_stored_key_on_failure(self) -> None:
        """deregister_device() preserves the key when removal fails."""
    ...
    def test_deregister_device_returns_false_when_no_key_stored(self) -> None:
        """deregister_device() returns False when no session key is stored (device remains registered)."""
    ...
    def test_deregister_device_falls_back_to_device_token_when_no_account_token(self) -> None:
        """deregister_device() falls back to the device access token when account token unavailable."""
    ...
    def test_log_off_clears_tokens_but_preserves_session_key_on_deregister_failure(self) -> None:
        """log_off() clears local tokens even when deregistration fails, but preserves the session key."""
        ...
        with unittest.mock.patch.object(device_handler, "deregister_device", return_value=False) as mock_remove:
            result = device_handler.log_off("test@example.com")
...
def _make_nlziet_handler(use_device_flow: bool = True) -> NLZIETHandler:
    ...
    with unittest.mock.patch("resources.lib.addonsettings.AddonSettings.get_setting",
                             return_value=None):
        return NLZIETHandler(use_device_flow=use_device_flow)


class TestNlzietHeaders(unittest.TestCase):
    """Unit tests for NLZIET request-header construction."""

    def test_get_headers_omits_user_agent_for_web_flow(self) -> None:
        """Web flow omits User-Agent so UriHandler injects its modern default."""

        handler = _make_nlziet_handler(use_device_flow=False)
        headers = handler.get_headers()

        self.assertNotIn("User-Agent", headers)


    def test_get_headers_sends_okhttp_user_agent_for_device_flow(self) -> None:
        """Device flow sends the OkHttp User-Agent matching the Android TV app."""

        handler = _make_nlziet_handler(use_device_flow=True)
        headers = handler.get_headers()

        self.assertEqual(DEVICE_FLOW_USER_AGENT, headers.get("User-Agent"))


    def test_get_headers_with_token_adds_bearer(self) -> None:
        """get_headers(token) adds Authorization: Bearer to the returned dict."""

        handler = _make_nlziet_handler()
        headers = handler.get_headers("tok123")

        self.assertEqual(headers.get("Authorization"), "Bearer tok123")


    def test_get_headers_without_token_uses_stored_access_token(self) -> None:
        """get_headers() with no arg uses the internally stored access token."""
    ...
    def test_get_headers_without_token_omits_bearer_when_no_stored_token(self) -> None:
        """get_headers() omits Authorization when no token is stored and none given."""
    ...
    def test_get_headers_explicit_token_overrides_stored(self) -> None:
        """Explicit token arg takes precedence over the internally stored token."""

        handler = _make_nlziet_handler()
        handler._access_token = "stored-tok"
        headers = handler.get_headers("override-tok")

        self.assertEqual(headers.get("Authorization"), "Bearer override-tok")
...
class TestNlzietPollDeviceFlow(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.handler = _make_nlziet_handler(use_device_flow=True)
    ...
    def test_device_access_token_request_passes_through_channel_app_headers(
            ...
            mock_open: unittest.mock.MagicMock,
            _mock_set: unittest.mock.MagicMock) -> None:
        """_device_access_token_request() includes Nlziet-* headers from the channel's httpHeaders."""
        ...
        with unittest.mock.patch("resources.lib.addonsettings.AddonSettings.get_setting",
            ...
            handler = NLZIETHandler(
                ...
                http_headers={"Nlziet-AppName": "AndroidTv", "Nlziet-AppVersion": "5.65.5"})
    ...
    def test_device_access_token_request_sets_device_client_id_on_success(
        ...
        from tests.authentication.nlziethandler_mocks import MOCK_ACCESS_TOKEN
        from resources.lib.authentication.nlziethandler import (
            AUTH_CLIENT_ID_KEY, DEVICE_CLIENT_ID)
        mock_open.return_value = f'{{"access_token": "{MOCK_ACCESS_TOKEN}", "expires_in": 3600}}'
...
class TestNlzietSaveAndClearTokens(unittest.TestCase):
    ...
    def test_save_tokens_stores_access_token_in_get_headers(
            self,
            mock_time: unittest.mock.MagicMock,
            _mock_set: unittest.mock.MagicMock) -> None:
        """_save_tokens() stores the access token; get_headers() reflects it as Bearer."""
        ...
        self.assertEqual(self.handler.get_headers().get("Authorization"), "Bearer tok123")
    ...
    def test_clear_tokens_does_not_leave_authorization_in_headers(
        ...
        self.assertEqual(self.handler._access_token, "")
        self.assertNotIn("Authorization", self.handler.get_headers())
    ...
    def test_init_with_cached_token_does_not_mutate_http_headers_dict(
            ...
            mock_time: unittest.mock.MagicMock,
            _mock_set: unittest.mock.MagicMock) -> None:
        """Handler init loads the access token without mutating the http_headers dict."""
        ...
        with unittest.mock.patch(
            ...
            handler = NLZIETHandler(use_device_flow=False, http_headers=http_headers)

        self.assertNotIn("Authorization", http_headers)
        self.assertEqual(handler._access_token, MOCK_ACCESS_TOKEN)
        self.assertIn("Authorization", handler.get_headers())
    ...
    def test_set_profile_claim_updates_access_token(
            ...
            mock_open: unittest.mock.MagicMock,
            _mock_set: unittest.mock.MagicMock) -> None:
        """Authorization header reflects the profile token via get_headers(token) after set_profile_claim()."""
        ...
        self.assertEqual(self.handler._access_token, MOCK_PROFILE_ACCESS_TOKEN)
        self.assertEqual(
            self.handler.get_headers(self.handler._access_token).get("Authorization"),
    ...
    def test_save_tokens_with_access_token_name_stores_access_token(
        ...
        self.assertIn("Authorization", self.handler.get_headers())
...
class TestNlzietLogOff(unittest.TestCase):
    """Unit tests for log_off() — deregistration and token cleanup."""
    ...
    def test_log_off_returns_true_when_deregistration_succeeds(
            self, _mock_set: unittest.mock.MagicMock) -> None:
        """log_off() returns True and clears tokens when device removal succeeds."""
        ...
        with unittest.mock.patch.object(self.handler, "deregister_device", return_value=True):
            result = self.handler.log_off("test@example.com")
    ...
    def test_log_off_returns_false_when_deregistration_fails(
            self, _mock_set: unittest.mock.MagicMock) -> None:
        """log_off() returns False and still clears tokens when device removal fails."""

        from tests.authentication.nlziethandler_mocks import MOCK_ACCESS_TOKEN
        self.handler._access_token = MOCK_ACCESS_TOKEN
        self.handler._id_token = "id_tok"

        with unittest.mock.patch.object(self.handler, "deregister_device", return_value=False):
            result = self.handler.log_off("test@example.com")
    ...
    @unittest.mock.patch("resources.lib.addonsettings.AddonSettings.set_setting")
    def test_log_off_headless_skips_deregistration(
            self, _mock_set: unittest.mock.MagicMock) -> None:
        """log_off() does not call deregister_device() for headless (non-device-flow) login."""

        headless_handler = _make_nlziet_handler(use_device_flow=False)

        with unittest.mock.patch.object(
                headless_handler, "deregister_device") as mock_deregister:
            result = headless_handler.log_off("test@example.com")
...
class TestNlzietStartDeviceFlow(unittest.TestCase):
    """Unit tests for the _device_authorization_request() NLZIET override."""
    ...
    def test_start_device_flow_sends_okhttp_user_agent(
        ...
        mock_open.return_value = '{"device_code": "d", "user_code": "U", "expires_in": 900}'

        self.handler.start_nlziet_device_flow(device_name="Test TV")
    ...
    def test_start_device_flow_returns_none_when_device_code_missing(
        ...
        result = self.handler.start_nlziet_device_flow(device_name="Test TV")
    ...
    def test_start_device_flow_returns_full_response_on_success(
        ...
        mock_open.return_value = '{"device_code": "dev123", "user_code": "AB12", "expires_in": 900}'
...
# ============================================================================
# Unit tests for _headless_login()
# ============================================================================


class TestNlzietHeadlessLogin(unittest.TestCase):
    """Unit tests for the NLZIET web login flow internals."""

    def setUp(self) -> None:
        self.handler = _make_nlziet_handler(use_device_flow=False)


    @unittest.mock.patch("resources.lib.urihandler.UriHandler.open")
    def test_headless_login_returns_network_error_when_return_url_missing(
            self, mock_open: unittest.mock.MagicMock) -> None:
        """_headless_login() returns network_error when the login redirect lacks ReturnUrl."""
        ...
        with unittest.mock.patch("resources.lib.urihandler.UriHandler.instance") as mock_instance:
            ...
            result = self.handler._headless_login("user@example.com", "pass")
...
# ============================================================================
# Unit tests for log_on() error types
# ============================================================================


class TestNlzietLogOnErrorTypes(unittest.TestCase):
    """Unit tests verifying log_on() surfaces the correct error strings."""

    def setUp(self) -> None:
        self.handler = _make_nlziet_handler()


    def test_log_on_empty_credentials_returns_missing_credentials(self) -> None:
        """log_on() with empty credentials returns error='missing_credentials'."""
    ...
    def test_log_on_invalid_credentials_returns_invalid_credentials(self) -> None:
        """log_on() surfaces 'invalid_credentials' when _headless_login fails."""

        with unittest.mock.patch.object(
            self.handler, "_headless_login", return_value="invalid_credentials"
        ):
            result = self.handler.log_on("user@example.com", "wrongpass")

        self.assertFalse(result.logged_on)
        self.assertEqual(result.error, "invalid_credentials")


    def test_log_on_network_error_returns_network_error(self) -> None:
        """log_on() surfaces 'network_error' when _headless_login fails."""

        with unittest.mock.patch.object(
            self.handler, "_headless_login", return_value="network_error"
        ):
            result = self.handler.log_on("user@example.com", "pass")

        self.assertFalse(result.logged_on)
        self.assertEqual(result.error, "network_error")


    def test_log_on_token_failure_returns_network_error(self) -> None:
        """log_on() returns 'network_error' when token fetch fails after headless login."""

        with unittest.mock.patch.object(self.handler, "_headless_login", return_value=None), \
             unittest.mock.patch("resources.lib.addonsettings.AddonSettings.set_setting"), \
...
class TestNLZIETActiveAuthentication(unittest.TestCase):
    ...
    def test_with_token_returns_existing_login(self) -> None:
        """active_authentication() returns existing_login=True with username from JWT."""
...
class TestNlzietLoginPageStructure(unittest.TestCase):
    ...
    upstream changes that would break the headless login flow.
    """
    ...
    def _fetch_login_page(self) -> tuple:
        ...
        html = UriHandler.open(auth_url, no_cache=True,
                               additional_headers=self.handler.get_headers())

...
class TestOIDCExtractUsername(unittest.TestCase):
    ...
    def test_falls_back_to_base_for_empty_token(self) -> None:
        """Returns empty string when token is empty."""

        self.assertEqual(self.handler._get_username_from_access_token(""), "")

    def test_falls_back_to_base_when_no_identity_claims(self) -> None:
        """Returns empty string when payload carries none of the known OIDC identity fields."""
...
class TestOIDCDoTokenRefresh(unittest.TestCase):
    ...
    @unittest.mock.patch("resources.lib.addonsettings.AddonSettings.set_setting")
    def test_silent_reauth_failure_clears_id_token_but_returns_true(
            self, _mock_set: unittest.mock.MagicMock) -> None:
        """_refresh_token_grant() returns True but clears the stale id_token when silent re-auth fails.

        The base refresh succeeded so the access token is valid; only the
        id_token renewal failed.  The overall refresh is still considered
        successful — the caller receives a working access token.
        """
...
class TestOIDCDoSilentReauth(unittest.TestCase):
    ...
    @unittest.mock.patch("resources.lib.addonsettings.AddonSettings.set_setting")
    @unittest.mock.patch("resources.lib.authentication.oidchandler.UriHandler")
    def test_returns_false_and_leaves_tokens_intact_on_failure(
            self, mock_uri: unittest.mock.MagicMock,
            _mock_set: unittest.mock.MagicMock) -> None:
        """_silent_authentication() returns False on failure without touching existing tokens."""
...
class TestOIDCEndSession(unittest.TestCase):
    """Unit tests for OIDCHandler._end_session()."""

    REVOCATION_URL = "https://auth.example.com/revoke"
    END_SESSION_URL = "https://auth.example.com/endsession"
    REDIRECT_URI = "https://localhost/logout"
    CLIENT_ID = "test-client"

    def _make_uri_status(self, error: bool = False, code: int = 200) -> unittest.mock.MagicMock:
        return unittest.mock.MagicMock(error=error, code=code, reason="OK")

    @unittest.mock.patch("resources.lib.urihandler.UriHandler.open")
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.instance")
    def test_revokes_access_token_and_ends_session(
            self, mock_instance: unittest.mock.MagicMock,
            mock_open: unittest.mock.MagicMock) -> None:
        """Posts to revocation and GETs end_session when both tokens are present."""

        mock_instance.return_value.status = self._make_uri_status(error=False)
        handler = _make_oidc_handler()
        handler._access_token = "acc_tok"
        handler._id_token = "id_tok"

        result = handler._end_session(
            self.CLIENT_ID, self.REVOCATION_URL, self.END_SESSION_URL, self.REDIRECT_URI)

        self.assertTrue(result)
        self.assertEqual(mock_open.call_count, 2)
        revoke_call = mock_open.call_args_list[0]
        self.assertEqual(revoke_call.args[0], self.REVOCATION_URL)
        end_session_call = mock_open.call_args_list[1]
        self.assertIn(self.END_SESSION_URL, end_session_call.args[0])
        self.assertIn("id_token_hint=id_tok", end_session_call.args[0])


    @unittest.mock.patch("resources.lib.urihandler.UriHandler.open")
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.instance")
    def test_skips_revocation_when_no_access_token(
            self, mock_instance: unittest.mock.MagicMock,
            mock_open: unittest.mock.MagicMock) -> None:
        """Does not POST to revocation when access_token is empty."""
        ...
        self.assertIn(self.END_SESSION_URL, mock_open.call_args.args[0])


    @unittest.mock.patch("resources.lib.urihandler.UriHandler.open")
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.instance")
    def test_skips_end_session_when_no_id_token(
            self, mock_instance: unittest.mock.MagicMock,
            mock_open: unittest.mock.MagicMock) -> None:
        """Does not GET end_session when id_token is empty."""
        ...
        self.assertTrue(result)
        self.assertEqual(mock_open.call_count, 1)
        self.assertEqual(mock_open.call_args.args[0], self.REVOCATION_URL)


    @unittest.mock.patch("resources.lib.urihandler.UriHandler.open")
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.instance")
    def test_returns_false_when_revocation_fails(
            self, mock_instance: unittest.mock.MagicMock,
            mock_open: unittest.mock.MagicMock) -> None:
        """Returns False when the revocation endpoint reports an error."""

        mock_instance.return_value.status = self._make_uri_status(error=True, code=400)
        handler = _make_oidc_handler()
        handler._access_token = "acc_tok"
        handler._id_token = ""
    ...
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.open")
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.instance")
    def test_no_calls_when_no_tokens(
            self, mock_instance: unittest.mock.MagicMock,
            mock_open: unittest.mock.MagicMock) -> None:
        """Makes no HTTP calls when both access_token and id_token are empty."""
    ...
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.open")
    @unittest.mock.patch("resources.lib.urihandler.UriHandler.instance")
    def test_returns_false_when_end_session_fails(
            self, mock_instance: unittest.mock.MagicMock,
            mock_open: unittest.mock.MagicMock) -> None:
        """Returns False when the end_session endpoint reports an error."""

        mock_instance.return_value.status = self._make_uri_status(error=True, code=500)
        handler = _make_oidc_handler()
        handler._access_token = ""
        handler._id_token = "id_tok"

...
# Optional dependency for user-agent fetching
try:
    import requests
except ImportError:
    requests = None
...
class NLZIETOAuth2Handler(OAuth2Handler):
    ...
    def list_profiles(self) -> Optional[list]:
        ...
        try:
            ...
            # Handle unauthorized responses by attempting token refresh and retry once
            if status.error and status.code in (HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN):
                ...
                try:
                    # Attempt to refresh tokens (may raise)
                    ...
                    # Refresh failed — clear stored tokens and signal not logged on

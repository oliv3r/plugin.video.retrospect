    ...
    def decode(token: str, **_kwargs: Any) -> dict:
        ...
        try:
            ...
            if not isinstance(claims, dict):
                raise ValueError(f"JWT payload is not a JSON object")
            return claims
...
class OAuth2Handler(AuthenticationHandler, ABC):
    ...
    def __init__(self, realm: str, client_id: str,
        ...
        :param headers:   Initial set of headers for authentication requests.
        """
    ...
    def redirect_uri(self) -> str:
        """
        Redirect URI (RFC 6749, §3.1.2) for the interactive authorization code flow.
    ...
    def scopes(self) -> List[str]:
        """
        Default OAuth2 scopes; override in subclass for provider-specific needs.
    ...
    def token_endpoint(self) -> str:
        """
        Token endpoint URL (RFC 6749, §3.2) for code and token exchange.
    ...
    def _token_request_headers(self) -> dict:
        """
        Return base HTTP headers for token endpoint requests.

        Override in subclasses to inject provider-specific headers into all
        token endpoint calls made by this class.
        The return value is used as the base dict; caller-supplied headers
        are merged on top.

        :return: Header dict for token endpoint requests (empty by default).
        """
    ...
    def _fetch_tokens(self, data: dict,
        ...
        if tokens.get("error"):
            description = tokens.get("error_description") or "no description"
            Logger.error(f"OAuth2: Token response for {self.realm} has error: {tokens["error"]!r} - {description!r}")
            return None
    ...
    def _request_token(self, data: dict, headers: Optional[dict] = None) -> bool:
        ...
        Applies :meth:`_token_request_headers` as the base so every grant path
        (code exchange, refresh, etc.) gets the correct base headers.
        ...
        request_headers = self._token_request_headers()
        if headers:
            request_headers.update(headers)
        tokens = self._fetch_tokens(data, request_headers)
    ...
    def _prefix(self) -> str:
        """Settings key prefix for all token fields belonging to this realm and client."""

        return f"{self._realm}_oauth2_{self._client_id}_"


    def _token_key(self, name: str) -> str:
        """
        Build the settings storage key for a token field.

        :param name: Token field name (e.g. ``"access_token"``).
        :return: Settings key string unique to this realm and client.
        """
    ...
    def _load_tokens(self, token_name: Optional[str] = None) -> dict:
        ...
        :return: Dict of token fields that were loaded.
        """
        ...
        if (token_name is None or
            ...
            self._access_token = (
                AddonSettings.get_setting(self._token_key("access_token"), store=LOCAL) or "")
            tokens["access_token"] = self._access_token

            self._access_token_expires_at = int(
                AddonSettings.get_setting(self._token_key("access_token_expires_at"), store=LOCAL) or 0)
            tokens["access_token_expires_at"] = self._access_token_expires_at

        if (token_name is None or
            token_name == "refresh_token"):
            self._refresh_token = (
                AddonSettings.get_setting(self._token_key("refresh_token"), store=LOCAL) or "")
    ...
    def _save_tokens(self, tokens: dict, token_name: Optional[str] = None) -> None:
        ...
        if ("access_token" in tokens and
            ...
            AddonSettings.set_setting(self._token_key("access_token"), self._access_token, store=LOCAL)

            expires_in = tokens.get("expires_in", ACCESS_TOKEN_EXPIRY_DEFAULT)
            self._access_token_expires_at = int(time.time()) + expires_in
            AddonSettings.set_setting(self._token_key("access_token_expires_at"),
        ...
        if ("refresh_token" in tokens and
            ...
            AddonSettings.set_setting(self._token_key("refresh_token"), self._refresh_token, store=LOCAL)
    ...
    def _clear_tokens(self, token_name: Optional[str] = None) -> None:
        ...
        if (token_name is None or
            ...
            self._access_token = ""
            self._access_token_expires_at = 0
            AddonSettings.set_setting(self._token_key("access_token"), "", store=LOCAL)
            AddonSettings.set_setting(self._token_key("access_token_expires_at"), "0", store=LOCAL)

        if (token_name is None or
            token_name == "refresh_token"):
            self._refresh_token = ""
            AddonSettings.set_setting(self._token_key("refresh_token"), "", store=LOCAL)
    ...
    def _refresh_token_grant(self) -> bool:
        ...
        :return: - True when a refresh token was retrieved.
    ...
    def _authorization_bearer(self, token: Optional[str] = None) -> dict:
        ...
        :return: - A dict containing the ``Authorization: Bearer`` header.
                 - An empty dict when no token is available.
        """
